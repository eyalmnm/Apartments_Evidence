package md.starlab.apartmentsevidenceapp.ui_widgets;
...
public class TwoDScrollView extends FrameLayout {
    ...
    /**
     * Holding the view's scroll listener
     */
    private OnTwoDScrollListener listener;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                Log.d("TAG", "current x: " + getScrollX() + " current y: " + getScrollY());
                Log.d("TAG", "height: " + computeVerticalScrollRange() + " width: " + computeHorizontalScrollRange());

                // Inform the listener
                if (null != listener) {
                    listener.onTwoDScroll(getScrollX(), getScrollY());
                    listener.onRelativeScroll(getScrollX() / (computeHorizontalScrollRange() * 1F), getScrollY() / (computeVerticalScrollRange() * 1F));
                }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (null != listener) {
                    listener.onLastScroll(getScrollX(), getScrollY());
                }
        }
        return true;
    }
     ...
     *
     * @param direction_vert or direction_horz the scroll direction: {@link android.view.View#FOCUS_UP}
     ...
     *
     * @param deltaX or deltaY the number of pixels to scroll by on the Y axis
     */
    ...
    public void setOnTwoDScrollListener(OnTwoDScrollListener listener) {
        this.listener = listener;
    }

    public interface OnTwoDScrollListener {
        void onTwoDScroll(int x, int y);

        void onRelativeScroll(float x, float y);

        void onLastScroll(int x, int y);
    }
}
